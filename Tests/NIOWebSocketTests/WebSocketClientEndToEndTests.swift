...
private final class BasicHTTPHandler: ChannelInboundHandler, RemovableChannelHandler {
    ...
    fileprivate func channelActive(context: ChannelHandlerContext) {
        ...
        fireSendRequest(context: context)
    }
}
...
private final class ExplodingHTTPHandler: ChannelInboundHandler, RemovableChannelHandler {
    ...
    fileprivate func errorCaught(context _: ChannelHandlerContext, error _: Error) {
...
class WebSocketClientEndToEndTests: XCTestCase {
    ...
    func testSendAFewFrames() throws {
        let (clientChannel, _) = try runSuccessfulUpgrade()
    ...
    private func encodeFrame(dataString: String, opcode _: WebSocketOpcode) throws -> (WebSocketFrame, [UInt8]) {
    ...
    func testReceiveAFewFrames() throws {
        let (clientChannel, recorder) = try runSuccessfulUpgrade()
        defer {
        ...
        let (binaryFrame, binaryFrameAsBytes) = try encodeFrame(dataString: "hello, back", opcode: .binary)
        ...
        // Frame number two coming up.
        let (textFrame, textFrameAsBytes) = try encodeFrame(dataString: "two", opcode: .text)
