...
class HTTPDecoderTest: XCTestCase {
    ...
    func testDoesNotDecodeRealHTTP09Request() throws {
        ...
        XCTAssertThrowsError(try self.channel.writeInbound(buffer)) { error in
    ...
    func testDoesNotDecodeRealHTTP09Response() throws {
        ...
        try self.channel.writeOutbound(HTTPClientRequestPart.head(HTTPRequestHead(version: .http0_9, method: .GET, uri: "/")))
    ...
    func testDoesNotDecodeHTTP2XResponse() throws {
        ...
        // We need to prime the decoder by seeing a GET request.
        try self.channel.writeOutbound(HTTPClientRequestPart.head(HTTPRequestHead(version: .http2, method: .GET, uri: "/")))
    ...
    func testCorrectlyMaintainIndicesWhenDiscardReadBytes() throws {
        ...
        var written = 0
        repeat {
            var buffer2 = self.channel.allocator.buffer(capacity: 16)
        ...
        XCTAssertNoThrow(try self.channel.writeInbound(buffer3))
        XCTAssertNoThrow(try self.channel.finish())
    }
    ...
    func testDropExtraBytes() throws {
        ...
        XCTAssertNoThrow(try
            self.channel.pipeline.addHandler(ByteToMessageHandler(HTTPRequestDecoder()),
                                             name: "decoder").wait())
        XCTAssertNoThrow(try self.channel.pipeline.addHandler(Receiver()).wait())
        ...
        XCTAssertNoThrow(try self.channel.writeInbound(buffer))
        (self.channel.eventLoop as! EmbeddedEventLoop).run() // allow the event loop to run (removal is not synchronous here)
        XCTAssertNoThrow(try self.channel.pipeline.assertDoesNotContain(handlerType: ByteToMessageHandler<HTTPRequestDecoder>.self))
        XCTAssertNoThrow(try self.channel.finish())
    }
    ...
    func testDontDropExtraBytesRequest() throws {
        ...
        XCTAssertNoThrow(try
            self.channel.pipeline.addHandler(ByteToMessageHandler(HTTPRequestDecoder(leftOverBytesStrategy: .forwardBytes)),
                                             name: "decoder").wait())
        XCTAssertNoThrow(try self.channel.pipeline.addHandler(Receiver()).wait())
        ...
        XCTAssertNoThrow(try self.channel.writeInbound(buffer))
        (self.channel.eventLoop as! EmbeddedEventLoop).run() // allow the event loop to run (removal is not synchrnous here)
        XCTAssertNoThrow(try self.channel.pipeline.assertDoesNotContain(handlerType: ByteToMessageHandler<HTTPRequestDecoder>.self))
        XCTAssertNoThrow(try self.channel.finish())
    }
    ...
    func testDontDropExtraBytesResponse() throws {
        ...
        XCTAssertNoThrow(try self.channel.pipeline.addHandler(ByteToMessageHandler(HTTPResponseDecoder(leftOverBytesStrategy: .forwardBytes)),
                                                              name: "decoder").wait())
        XCTAssertNoThrow(try self.channel.pipeline.addHandler(Receiver()).wait())
    ...
    func testExtraCRLF() throws {
        ...
        let secondMessage: HTTPServerRequestPart? = try channel.readInbound()
    ...
    func testRefusesRequestSmugglingAttempt() throws {
        ...
        var buffer = self.channel.allocator.buffer(capacity: 256)
    ...
    func testTrimsTrailingOWS() throws {
        ...
        guard case .some(.end) = try assertNoThrowWithValue(self.channel.readInbound(as: HTTPServerRequestPart.self)) else {
