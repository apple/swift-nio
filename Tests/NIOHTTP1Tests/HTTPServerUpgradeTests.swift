...
extension ChannelPipeline {
    ...
    func assertContains<Handler: ChannelHandler>(handlerType: Handler.Type) throws {
        XCTAssertNoThrow(try context(handlerType: handlerType).wait(), "did not find handler")
    }

    fileprivate func removeUpgrader() throws {
        try context(handlerType: HTTPServerUpgradeHandler.self).flatMap {
...
private class DataRecorder<T>: ChannelInboundHandler {
    ...
    public func channelRead(context _: ChannelHandlerContext, data: NIOAny) {
        let datum = unwrapInboundIn(data)
...
class HTTPServerUpgradeTestCase: XCTestCase {
    ...
    func testUpgradeFiresUserEvent() throws {
        ...
        let upgrader = SuccessfulUpgrader(forProtocol: "myproto", requiringHeaders: []) { _ in
    ...
    func testUpgradeIsCaseInsensitive() throws {
        let upgrader = SuccessfulUpgrader(forProtocol: "myproto", requiringHeaders: ["WeIrDcAsE"]) { _ in }
        let (group, _, client, connectedServer) = try setUpTestWithAutoremoval(upgraders: [upgrader],
                                                                               extraHandlers: []) { context in
            context.close(promise: nil)
        }
    ...
    func testBuffersInboundDataDuringDelayedUpgrade() throws {
        ...
        let (group, server, client, _) = try setUpTestWithAutoremoval(upgraders: [upgrader],
                                                                      extraHandlers: [dataRecorder]) { _ in
            g.leave()
        }
        defer {
            XCTAssertNoThrow(try group.syncShutdownGracefully())
    ...
    func testDelayedUpgradeResponse() throws {
        ...
        XCTAssertNoThrow(try channel.pipeline.configureHTTPServerPipeline(withServerUpgrade: (upgraders: [delayedUpgrader], completionHandler: { _ in })).wait())
    ...
    func testChainsDelayedUpgradesAppropriately() throws {
        ...
        XCTAssertNoThrow(try channel.pipeline.configureHTTPServerPipeline(withServerUpgrade: (upgraders: [myprotoUpgrader, failingProtocolUpgrader], completionHandler: { _ in })).wait())
    ...
    func testDelayedUpgradeResponseDeliversFullRequestAndPendingBits() throws {
        ...
        XCTAssertNoThrow(try channel.pipeline.configureHTTPServerPipeline(withPipeliningAssistance: false,
                                                                          withServerUpgrade: (upgraders: [delayedUpgrader], completionHandler: { _ in })).wait())
    ...
    func testDeliversBytesWhenRemovedDuringPartialUpgrade() throws {
        ...
        XCTAssertNoThrow(try channel.pipeline.configureHTTPServerPipeline(withServerUpgrade: (upgraders: [delayer], completionHandler: { _ in })).wait())
