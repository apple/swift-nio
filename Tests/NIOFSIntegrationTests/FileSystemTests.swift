...
final class FileSystemTests: XCTestCase {
    ...
    func testSplitContentsOfFile() async throws {
        let fs = FileSystem.shared
        ...
        try await fs.withFileHandle(forReadingAt: path) { file in
            ...
            for try await var characters in file.readChunks(chunkLength: .bytes(1)).split(
    ...
    func testSplitContentsOfFileNotOmittingEmptySubsequences() async throws {
        let fs = FileSystem.shared
        let path = try await fs.temporaryFilePath()
