...
class UniversalBootstrapSupportTest: XCTestCase {
    func testBootstrappingWorks() {
        ...
        XCTAssertNoThrow(try withTCPServerChannel(group: group) { server in
            ...
            var maybeAcceptedChannel: Channel?
    ...
    func testBootstrapOverrideOfShortcutOptions() {
        final class FakeBootstrap: NIOClientTCPBootstrapProtocol {
            func channelInitializer(_: @escaping (Channel) -> EventLoopFuture<Void>) -> Self {
                fatalError("Not implemented")
            }

            func protocolHandlers(_: @escaping () -> [ChannelHandler]) -> Self {
                fatalError("Not implemented")
            }
            ...
            func connectTimeout(_: TimeAmount) -> Self {
                fatalError("Not implemented")
            }

            func connect(host _: String, port _: Int) -> EventLoopFuture<Channel> {
                fatalError("Not implemented")
            }

            func connect(to _: SocketAddress) -> EventLoopFuture<Channel> {
                fatalError("Not implemented")
            }

            func connect(unixDomainSocketPath _: String) -> EventLoopFuture<Channel> {
                fatalError("Not implemented")
            }
        }
