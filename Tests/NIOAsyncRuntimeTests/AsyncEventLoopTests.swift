...
final class AsyncEventLoopGroupTests {
    ...
    func testScheduledRepeatedAsyncTaskIsJittered() async throws {
        ...
        _ = loop.scheduleRepeatedAsyncTask(
            ...
            { _ in
                ...
                _ = loop.scheduleTask(in: completionDelay) {
                    p.succeed(())
                }
                return p.futureResult
            }
        ...
        try await loop.__testOnly_advanceTime(by: initialDelay - .nanoseconds(1))
        #expect(counter.load(ordering: .relaxed) == 0)
        try await loop.__testOnly_advanceTime(by: maximumAllowableJitter + .nanoseconds(1))
        #expect(counter.load(ordering: .relaxed) == 1)

        var uncertainty = maximumAllowableJitter
        for expectedExecutionCount in 2...4 {
            try await loop.__testOnly_advanceTime(
                by: delay + completionDelay - uncertainty - .nanoseconds(1)
            )
            #expect(counter.load(ordering: .relaxed) == Int64(expectedExecutionCount - 1))
            try await loop.__testOnly_advanceTime(by: uncertainty + maximumAllowableJitter + .nanoseconds(1))
            #expect(counter.load(ordering: .relaxed) == Int64(expectedExecutionCount))
            uncertainty += maximumAllowableJitter
        }
    }
    ...
    func testRepeatedTaskIsJittered() async throws {
        ...
        var uncertainty = maximumAllowableJitter
        for expectedExecutionCount in 2...4 {
            try await loop.__testOnly_advanceTime(by: delay - uncertainty - .nanoseconds(1))
            #expect(counter.load(ordering: .relaxed) == Int64(expectedExecutionCount - 1))
            try await loop.__testOnly_advanceTime(by: uncertainty + maximumAllowableJitter + .nanoseconds(1))
            #expect(counter.load(ordering: .relaxed) == Int64(expectedExecutionCount))
            uncertainty += maximumAllowableJitter
