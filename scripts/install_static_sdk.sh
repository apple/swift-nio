...
apt-get update > /dev/null && apt-get install -y curl rsync jq > /dev/null
...
if [[ -n "$branch" ]]; then
  ...
  snapshots="$(curl -s "https://www.swift.org/api/v1/install/dev/main/ubuntu2204.json" | jq -r --arg arch $arch '.[$arch] | unique | reverse | .[].dir')"
  for snapshot in $snapshots; do
    snapshot_url="https://download.swift.org/development/ubuntu2204${arch_suffix}/${snapshot}/${snapshot}-ubuntu22.04${arch_suffix}.tar.gz"
  ...
  snapshot_url="https://download.swift.org/swift-${version}-release/ubuntu2204${arch_suffix}/swift-${version}-RELEASE/swift-${version}-RELEASE-ubuntu22.04${arch_suffix}.tar.gz"
