...
/// `NIOAsyncTestingChannel.writeInbound` which accepts data of any type. It will then
...
///
/// `NIOAsyncTestingChannel` automatically collects arriving outbound data and makes it
...
public final class NIOAsyncTestingChannel: Channel {
    ...
    /// outbound data to verify they are what you expect. Therefore, when you `finish` an `NIOAsyncTestingChannel` it will
    /// return if it's either `.clean` (no left overs) or that it has `.leftOvers`.
    public enum LeftOverState {
        /// The `NIOAsyncTestingChannel` is clean, ie. no inbound, outbound, or pending outbound data left on `finish`.
        case clean
        ...
        /// `true` if the `NIOAsyncTestingChannel` was `clean` on `finish`, ie. there is no unconsumed inbound, outbound, or
    ...
    /// `BufferState` represents the state of either the inbound, or the outbound `NIOAsyncTestingChannel` buffer. These
    ...
    /// first element was of a different type than requested, `NIOAsyncTestingChannel.WrongTypeError` will be thrown, if there
