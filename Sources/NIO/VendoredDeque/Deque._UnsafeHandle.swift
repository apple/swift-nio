...
extension NIODeque {
  ...
  internal struct _UnsafeHandle {
    @usableFromInline
    let _header: UnsafeMutablePointer<_NIODequeBufferHeader>
    @usableFromInline
    ...
    @inline(__always)
    init(
      header: UnsafeMutablePointer<_NIODequeBufferHeader>,
...
extension NIODeque._UnsafeHandle {
  /* was @inlinable */
  @inline(__always)
  func assertMutable() {
    #if DEBUG
...
extension NIODeque._UnsafeHandle {
  @usableFromInline
  internal typealias Slot = _NIODequeSlot

  /* was @inlinable */
  @inline(__always)
  var header: _NIODequeBufferHeader {
    _header.pointee
  }
...
extension NIODeque._UnsafeHandle {
  ...
  @inline(__always)
  @discardableResult
  @usableFromInline internal /*was public */ func move(
    from source: Slot,
    to target: Slot,
    count: Int
...
extension NIODeque._UnsafeHandle {
  ...
  /// layout.
  /* was @inlinable */
  internal func copyElements() -> NIODeque._Storage {
    let object = _NIODequeBuffer<Element>.create(
      minimumCapacity: capacity,
      makingHeaderWith: { _ in header })
    let result = NIODeque._Storage(_buffer: ManagedBufferPointer(unsafeBufferObject: object))
  ...
  /// capacity.
  /* was @inlinable */
  internal func copyElements(minimumCapacity: Int) -> NIODeque._Storage {
    assert(minimumCapacity >= count)
    let object = _NIODequeBuffer<Element>.create(
      minimumCapacity: minimumCapacity,
      makingHeaderWith: {
        _NIODequeBufferHeader(
          capacity: $0.capacity,
          count: count,
          startSlot: .zero)
      })
  ...
  /* was @inlinable */
  internal func moveElements(minimumCapacity: Int) -> NIODeque._Storage {
    assertMutable()
