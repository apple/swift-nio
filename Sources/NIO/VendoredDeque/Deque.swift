...
/// A collection implementing a double-ended queue. `NIODeque` (pronounced "deck")
/// implements an ordered random-access collection that supports efficient
/// insertions and removals from both ends.
///
///     var colors: NIODeque = ["red", "yellow", "blue"]
///
/// NIODeques implement the same indexing semantics as arrays: they use integer
...
///     // ["red", "green", "blue"]
///
/// Like all variable-size collections on the standard library, `NIODeque`
...
///
/// `NIODeque` stores its elements in a circular buffer, which allows efficient
...
///
/// NIODeques provide a handful of additional operations that make it easier to
...
/* was @frozen */
@usableFromInline internal /*was public */ struct NIODeque<Element> {
  @usableFromInline
  internal typealias _Slot = _NIODequeSlot
  ...
  ///   storage buffer.
  /* was @inlinable */
  @usableFromInline internal /*was public */ init(minimumCapacity: Int) {
