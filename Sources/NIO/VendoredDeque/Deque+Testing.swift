...
extension NIODeque {
  ...
  ///
  /// This property isn't intended to be used outside of `NIODeque`'s own test
  /// target.
  
  @usableFromInline internal /*was public */ var _capacity: Int {
    _storage.capacity
  }
  ...
  @usableFromInline internal /*was public */ var _startSlot: Int {
    _storage.startSlot.position
  }

  /// Constructs a deque instance of the specified contents and layout. Exposed
  /// as public to allow exhaustive input/output tests for `NIODeque`'s members.
  /// This isn't intended to be used outside of `NIODeque`'s own test target.
  
  @usableFromInline internal /*was public */ init<S: Sequence>(
    ...
    let buffer = _NIODequeBuffer<Element>.create(minimumCapacity: capacity) { _ in
      _NIODequeBufferHeader(capacity: capacity, count: contents.count, startSlot: startSlot)
    }
    let storage = NIODeque<Element>._Storage(unsafeDowncast(buffer, to: _NIODequeBuffer.self))
    if contents.count > 0 {
