...
//===----------------------------------------------------------------------===//
        ...
        extension Selector: _SelectorBackendProtocol {
            func initialiseState0() throws {
                selectorFD = try Epoll.epoll_create(size: 128)
                ...
                try Epoll.epoll_ctl(epfd: selectorFD, op: Epoll.EPOLL_CTL_ADD, fd: eventFD, event: &ev)
                ...
                try Epoll.epoll_ctl(epfd: selectorFD, op: Epoll.EPOLL_CTL_ADD, fd: timerFD, event: &timerev)
            }

            func deinitAssertions0() {
                assert(eventFD == -1, "self.eventFD == \(eventFD) in deinitAssertions0, forgot close?")
                assert(timerFD == -1, "self.timerFD == \(timerFD) in deinitAssertions0, forgot close?")
            }

            func register0<S: Selectable>(selectable _: S,
                                          fileDescriptor: CInt,
                                          ...
                                          registrationID: SelectorRegistrationID) throws
            {
                ...
                try Epoll.epoll_ctl(epfd: selectorFD, op: Epoll.EPOLL_CTL_ADD, fd: fileDescriptor, event: &ev)
            }

            func reregister0<S: Selectable>(selectable _: S,
                                            fileDescriptor: CInt,
                                            oldInterested _: SelectorEventSet,
                                            ...
                                            registrationID: SelectorRegistrationID) throws
            {
                ...
                _ = try Epoll.epoll_ctl(epfd: selectorFD, op: Epoll.EPOLL_CTL_MOD, fd: fileDescriptor, event: &ev)
            }

            func deregister0<S: Selectable>(selectable _: S, fileDescriptor: CInt, oldInterested _: SelectorEventSet, registrationID _: SelectorRegistrationID) throws {
                var ev = Epoll.epoll_event()
                _ = try Epoll.epoll_ctl(epfd: selectorFD, op: Epoll.EPOLL_CTL_DEL, fd: fileDescriptor, event: &ev)
            }
            ...
            func whenReady0(strategy: SelectorStrategy, onLoopBegin loopStart: () -> Void, _ body: (SelectorEvent<R>) throws -> Void) throws {
                ...
                switch strategy {
                case .now:
                    ready = Int(try Epoll.epoll_wait(epfd: selectorFD, events: events, maxevents: Int32(eventsCapacity), timeout: 0))
                case let .blockUntilTimeout(timeAmount):
                    ...
                    fallthrough
                case .block:
                    ready = Int(try Epoll.epoll_wait(epfd: selectorFD, events: events, maxevents: Int32(eventsCapacity), timeout: -1))
                }
                ...
                for i in 0 ..< ready {
                    ...
                    case timerFD:
                        ...
                        _ = try! Posix.read(descriptor: timerFD, pointer: &val, size: MemoryLayout.size(ofValue: val))

                        // Processed the earliest set timer so reset it.
                        earliestTimer = .distantFuture
                    default:
