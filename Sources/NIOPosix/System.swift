...
private let sysSendMmsg: @convention(c) (CInt, UnsafeMutablePointer<CNIOLinux_mmsghdr>?, CUnsignedInt, CInt) -> CInt =
    CNIOLinux_sendmmsg
private let sysRecvMmsg:
    @convention(c) (CInt, UnsafeMutablePointer<CNIOLinux_mmsghdr>?, CUnsignedInt, CInt, UnsafeMutablePointer<timespec>?)
        -> CInt = CNIOLinux_recvmmsg
#elseif canImport(Darwin)
private let sysKevent = kevent
private let sysSendMmsg: @convention(c) (CInt, UnsafeMutablePointer<CNIODarwin_mmsghdr>?, CUnsignedInt, CInt) -> CInt =
    CNIODarwin_sendmmsg
private let sysRecvMmsg:
    @convention(c) (
        CInt, UnsafeMutablePointer<CNIODarwin_mmsghdr>?, CUnsignedInt, CInt, UnsafeMutablePointer<timespec>?
    ) -> CInt = CNIODarwin_recvmmsg
#endif
#if !os(Windows)
#if canImport(Musl)
...
private func preconditionIsNotUnacceptableErrnoOnClose(err: CInt, where function: String) {
    // strerror is documented to return "Unknown error: ..." for illegal value so it won't ever fail
    #if os(Windows)
...
internal enum Posix {
    ...
    @inline(never)
    internal static func setsockopt(
        socket: CInt,
        level: CInt,
        optionName: CInt,
    ...
    @inline(never)
    internal static func getsockopt(
        socket: CInt,
        level: CInt,
        optionName: CInt,
    ...
    @inline(never)
    internal static func getpeername(
        socket: CInt,
    ...
    @inline(never)
    internal static func getsockname(
        socket: CInt,
        address: UnsafeMutablePointer<sockaddr>,
        addressLength: UnsafeMutablePointer<socklen_t>
    ) throws {
    ...
    @inline(never)
    internal static func socketpair(
        domain: NIOBSDSocket.ProtocolFamily,
        type: NIOBSDSocket.SocketType,
        protocolSubtype: NIOBSDSocket.ProtocolSubtype,
