...
#if os(WASI) || (canImport(Testing) && !os(Android))
...
/// Errors that may be thrown when executing work on a `AsyncThreadPool`.
public enum AsyncThreadPoolError: Sendable {
...
/// Drop‑in for `NIOThreadPool` from NIOPosix, powered by Swift Concurrency.
///
/// - note: AsyncThreadPool and similar classes in NIOAsyncRuntime are not intended
...
public final class AsyncThreadPool: Sendable {
    ...
    private let shutdownFlag = Atomic(false)
    private let started = Atomic(false)
    private let numberOfThreads: Int
    private let workQueue = WorkQueue()
    private let workerTasks: NIOLockedValueBox<[Task<Void, Never>]> = NIOLockedValueBox([])
    ...
    public func submit(_ body: @escaping WorkItem) {
        ...
        Task {
            await workQueue.enqueue(IdentifiableWorkItem(workItem: body, id: nil))
        }
    }
    ...
    private func makeFutureByRunningOnPool<T: Sendable>(
        ...
        guard self.isActive else {
            return eventLoop.makeFailedFuture(AsyncThreadPoolError.ThreadPoolInactive())
        }
        ...
        self.submit { state in
            ...
            case .cancelled:
                promise.fail(AsyncThreadPoolError.ThreadPoolInactive())
            }
        }
    ...
    public static let singleton: AsyncThreadPool = {
    ...
    private func _shutdownGracefully(completion: (@Sendable () -> Void)? = nil) {
        ...
        Task {
            ...
            workerTasks.withLockedValue { mutableWorkerTasks in
                for worker in mutableWorkerTasks {
                    worker.cancel()
                }
                mutableWorkerTasks.removeAll()
            }
    ...
    private func spawnWorkers() {
        workerTasks.withLockedValue { mutableWorkerTasks in
            guard mutableWorkerTasks.isEmpty else { return }
    ...
    actor WorkQueue {
        ...
        func nextWorkItem(shutdownFlag: borrowing Atomic<Bool>) async -> IdentifiableWorkItem? {
            if !queue.isEmpty {
...
#endif  // os(WASI) || (canImport(Testing) && !os(Android))
