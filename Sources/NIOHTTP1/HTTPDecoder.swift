...
private class BetterHTTPParser {
    ...
    private func flush(
        _ callout: (inout HTTPDecoderDelegate, UnsafeRawBufferPointer) throws -> Void
    ) rethrows {
        let currentFieldByteLength = self.currentFieldByteLength
        ...
        precondition(
            start >= self.rawBytesView.startIndex &&
            end <= self.rawBytesView.endIndex
        )
    
        try callout(
            &self.delegate,
            .init(rebasing: self.rawBytesView[start..<end])
        )
    }
    private func finish(
        _ callout: (inout HTTPDecoderDelegate, UnsafeRawBufferPointer) throws -> Void
    ) rethrows {
        try self.flush(callout)
        self.currentFieldByteLength = 0
    }
    ...
    private func didReceiveHeaderValueData(_ bytes: UnsafeRawBufferPointer) -> CInt {
        do {
            ...
            case .headerValue, .trailerValue:
                ()
            case .headerName:
                try self.flush { delegate, bytes in
