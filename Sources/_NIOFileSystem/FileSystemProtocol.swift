...
public protocol FileSystemProtocol: Sendable {
    ...
    /// Copies the item at the specified path to a new location, optionally replacing an existing
    /// item at the destination.
    ///
    /// Behaves like ``copyItem(at:to:strategy:shouldProceedAfterError:shouldCopyItem:)`` but
    /// additionally supports atomically replacing an existing file at `destinationPath` when
