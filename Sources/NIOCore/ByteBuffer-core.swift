...
public struct ByteBuffer {
    ...
    mutating func _moveReaderIndex(to newIndex: _Index) {
        assert(newIndex >= 0 && newIndex <= self.writerIndex)
    ...
    @inlinable public var writableBytes: Int { Int(_toCapacity(self._slice.count) - self._writerIndex) }

    /// The number of bytes readable (`readableBytes` = `writerIndex` - `readerIndex`).
    @inlinable public var readableBytes: Int { Int(self._writerIndex - self._readerIndex) }
    ...
    var _slicedStorageBuffer: UnsafeMutableRawBufferPointer {
        UnsafeMutableRawBufferPointer(start: self._storage.bytes.advanced(by: Int(self._slice.lowerBound)),
...
extension ByteBuffer: CustomStringConvertible {
    ...
    public var debugDescription: String {
        "\(self.description)\nreadable bytes (max 1k): \(self._storage.dumpBytes(slice: self._slice, offset: self.readerIndex, length: min(1_024, self.readableBytes)))"
    }
}
